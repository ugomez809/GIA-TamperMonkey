...
// @updateURL    https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/Admins/Spam%20Guru%20Risk%20Ratings/ricochet-spam-guru-risk-ratings-updater.user.js
// @downloadURL  https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/Admins/Spam%20Guru%20Risk%20Ratings/ricochet-spam-guru-risk-ratings-updater.user.js
// ==/UserScript==
...
(function () {
  ...
  const TARGET_ID = 'ricochet-spam-guru-risk-ratings';
  const TARGET_LABEL = 'Ricochet Spam Guru Risk Ratings';
  const TARGET_FILE = 'ricochet-spam-guru-risk-ratings.user.js';
  const BASE_URL = 'https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/Admins/Spam%20Guru%20Risk%20Ratings';
  ...
  async function getTargetBaseUrl() {
    try {
      ...
      latestBaseUrl = `https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/${sha}/Admins/Spam%20Guru%20Risk%20Ratings`;
  ...
  function applyOptionsFromUrl() {
    ...
    debugEnabled = isTruthy(url.searchParams.get('ricochetUpdaterDebug')) || isTruthy(url.searchParams.get('ricochetSpamGuruUpdaterDebug'));
    forceRequested = isTruthy(url.searchParams.get('ricochetUpdaterForce')) || isTruthy(url.searchParams.get('ricochetSpamGuruUpdaterForce'));
    clearRequested = isTruthy(url.searchParams.get('ricochetUpdaterClear')) || isTruthy(url.searchParams.get('ricochetSpamGuruUpdaterClear'));

    if (forceRequested || clearRequested) sessionStorage.removeItem(RELOAD_KEY);

    url.searchParams.delete('ricochetSpamGuruUpdaterDebug');
    url.searchParams.delete('ricochetSpamGuruUpdaterForce');
    url.searchParams.delete('ricochetSpamGuruUpdaterClear');
