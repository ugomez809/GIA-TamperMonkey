// ==UserScript==
// @name         CFP Quick Quote PDF CPAGY Updater
// @namespace    local.cfp-quick-quote-pdf-cpagy.updater
// @version      0.1
// @description  Loads and auto-updates the CFP Quick Quote PDF CPAGY script from GitHub.
...
// @resource     cfpPdf https://gomezagency.net/monkey/cperez/cfpqq-CP-base64-code.txt
// @connect      api.github.com
// @connect      raw.githubusercontent.com
// @updateURL    https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/General%20Utilities/CFP%20Quick%20Quote%20PDF%20CPAGY/cfp-quick-quote-pdf-cpagy-updater.user.js
// @downloadURL  https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/General%20Utilities/CFP%20Quick%20Quote%20PDF%20CPAGY/cfp-quick-quote-pdf-cpagy-updater.user.js
// ==/UserScript==
...
(function () {
  ...
  const TARGET_ID = 'cfp-quick-quote-pdf-cpagy';
  const TARGET_LABEL = 'CFP Quick Quote PDF CPAGY';
  const TARGET_FILE = 'cfp-quick-quote-pdf-cpagy.user.js';
  const BASE_URL = 'https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/General%20Utilities/CFP%20Quick%20Quote%20PDF%20CPAGY';
  ...
  async function getTargetBaseUrl() {
    try {
      ...
      latestBaseUrl = `https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/${sha}/General%20Utilities/CFP%20Quick%20Quote%20PDF%20CPAGY`;
  ...
  function applyOptionsFromUrl() {
    ...
    debugEnabled = isTruthy(url.searchParams.get('staffUpdaterDebug')) || isTruthy(url.searchParams.get('cfpQuickQuoteCpagyUpdaterDebug'));
    forceRequested = isTruthy(url.searchParams.get('staffUpdaterForce')) || isTruthy(url.searchParams.get('cfpQuickQuoteCpagyUpdaterForce'));
    clearRequested = isTruthy(url.searchParams.get('staffUpdaterClear')) || isTruthy(url.searchParams.get('cfpQuickQuoteCpagyUpdaterClear'));
    ...
    url.searchParams.delete('cfpQuickQuoteCpagyUpdaterDebug');
    url.searchParams.delete('cfpQuickQuoteCpagyUpdaterForce');
    url.searchParams.delete('cfpQuickQuoteCpagyUpdaterClear');
  ...
  function isDuckCreek() {
    return /(^|\.)duckcreekondemand\.com$/i.test(String(location.hostname || ''));
  }
