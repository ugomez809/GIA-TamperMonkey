// ==UserScript==
// @name         CFP Quick Quote PDF CPAGY
// @namespace    GIA.Scripts
// @version      8.6
// @description  Generate the CPAGY CFP quick quote PDF.
// @author       Mr.G
...
// @updateURL    https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/General%20Utilities/CFP%20Quick%20Quote%20PDF%20CPAGY/cfp-quick-quote-pdf-cpagy.user.js
// @downloadURL  https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/General%20Utilities/CFP%20Quick%20Quote%20PDF%20CPAGY/cfp-quick-quote-pdf-cpagy.user.js
// ==/UserScript==
...
(function () {
   ...
   ********************************************************/
  console.log('[CFP CPAGY DEBUG] Using GM_getResourceText to load PDF base64.');
  const pdfTemplateBase64 = GM_getResourceText('cfpPdf') || '';
  console.log('[CFP CPAGY DEBUG] pdfTemplateBase64 length =', pdfTemplateBase64.length);

  if (!pdfTemplateBase64) {
    console.warn('[CFP CPAGY DEBUG] pdfTemplateBase64 is empty - PDF generation may fail if used.');
  }

  /********************************************************
   * 1) CREATE THE FLOATING "GENERATE CPAGY QUOTE" BUTTON
   ********************************************************/
  const button = document.createElement('button');
  button.innerText = 'Generate CPAGY Quote';
  ...
  function onCalculatePremiumClick() {
    console.log('[CFP CPAGY DEBUG] "Calculate Premium" button clicked.');
  ...
  function observeCompleteApplicationButton() {
    ...
    if (completeAppBtn) {
      completeAppBtn.addEventListener('click', () => {
        console.log('[CFP CPAGY DEBUG] "Complete an Application" clicked. Hiding button.');
        button.style.display = 'none';
        if (premiumCheckInterval) {
          clearInterval(premiumCheckInterval);
  ...
  modal.innerHTML = `
    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%;
      ...
      <div style="background: #ffffff; padding: 20px; border-radius: 8px; width: 300px; text-align: center;">
        ...
        <input type="text" id="cfpCpagyClientNameInput" placeholder="Client Name"
               style="width: 100%; padding: 8px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 4px;" />
        <button id="cfpCpagySubmitClientName"
  ...
  const submitButton = modal.querySelector('#cfpCpagySubmitClientName');
  submitButton.addEventListener('click', () => {
    const clientName = modal.querySelector('#cfpCpagyClientNameInput').value.trim();
    if (clientName) {
      modal.style.display = 'none';
  ...
  async function generatePDF(clientName) {
    try {
      ...
      link.download = `CFP_CPAGY_QQ_${clientName}_${currentDate}.pdf`;
      link.click();
    } catch (error) {
      console.error('[CFP CPAGY DEBUG] Error generating PDF:', error);
    }
  }
