...
(function () {
  ...
  const BASE_URL = 'https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/Ricochet/Call%20Script';
  ...
  function boot() {
    if (!isRicochet() && !(location.hostname === 'example.com' && location.hash === '#tm-auto-shop-script-window')) return;
    applyOptionsFromUrl();
  ...
  async function checkForUpdates(options = {}) {
    ...
    if (!executed) { executeTarget(remote, 'remote'); return; }
    if (!sameCode(remote, runningCode)) {
        ...
        .then((baseUrl) => {
          ...
          GM_xmlhttpRequest({
            ...
            onload: (response) => {
              ...
              if (!text.includes('// ==UserScript==') || !/^\/\/\s*@namespace\s+local\.ricochet-sdr-transfer-script-sync\s*$/m.test(text) || extractVersion(text) === 'unknown') {
  ...
  function reloadOnce(version, force) {
    // Do not interrupt a call or discard its guide notes for a background update.
    try {
      const payload = JSON.parse(GM_getValue('tmRicochetSdrV2Payload', 'null'));
    ...
    window.setTimeout(() => {
      try {
        if (JSON.parse(GM_getValue('tmRicochetSdrV2Payload', 'null'))?.leads?.length) {
  ...
  async function getTargetBaseUrl() {
    try {
      ...
      latestBaseUrl = `https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/${sha}/Ricochet/Call%20Script`;
  ...
  function applyOptionsFromUrl() {
    ...
    debugEnabled = isTruthy(url.searchParams.get('ricochetUpdaterDebug')) || isTruthy(url.searchParams.get('ricochetCallScriptUpdaterDebug'));
    forceRequested = isTruthy(url.searchParams.get('ricochetUpdaterForce')) || isTruthy(url.searchParams.get('ricochetCallScriptUpdaterForce'));
    clearRequested = isTruthy(url.searchParams.get('ricochetUpdaterClear')) || isTruthy(url.searchParams.get('ricochetCallScriptUpdaterClear'));

    if (forceRequested || clearRequested) sessionStorage.removeItem(RELOAD_KEY);

    url.searchParams.delete('ricochetCallScriptUpdaterDebug');
    url.searchParams.delete('ricochetCallScriptUpdaterForce');
    url.searchParams.delete('ricochetCallScriptUpdaterClear');
