// ==UserScript==
// @name         Ricochet Pickup & Hangup Counters Updater
// @namespace    local.ricochet-pickup-hangup-counters.updater
// @version      1.1.0
// @description  Loads and auto-updates only the Ricochet Pickup & Hangup Counters script from GitHub.
...
// @updateURL    https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/Ricochet/Pickup%20%26%20Hangup%20Counters/ricochet-pickup-hangup-counters-updater.user.js
// @downloadURL  https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/Ricochet/Pickup%20%26%20Hangup%20Counters/ricochet-pickup-hangup-counters-updater.user.js
// ==/UserScript==
...
(function () {
  ...
  const TARGET_ID = 'ricochet-pickup-hangup-counters';
  const TARGET_LABEL = 'Ricochet Pickup & Hangup Counters';
  const TARGET_FILE = 'ricochet-pickup-hangup-counters.user.js';
  const BASE_URL = 'https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/main/Ricochet/Pickup%20%26%20Hangup%20Counters';
  ...
  async function getTargetBaseUrl() {
    try {
      ...
      latestBaseUrl = `https://raw.githubusercontent.com/ugomez809/GIA-TamperMonkey/${sha}/Ricochet/Pickup%20%26%20Hangup%20Counters`;
  ...
  function applyOptionsFromUrl() {
    ...
    debugEnabled = isTruthy(url.searchParams.get('ricochetUpdaterDebug')) || isTruthy(url.searchParams.get('ricochetCountersUpdaterDebug'));
    forceRequested = isTruthy(url.searchParams.get('ricochetUpdaterForce')) || isTruthy(url.searchParams.get('ricochetCountersUpdaterForce'));
    clearRequested = isTruthy(url.searchParams.get('ricochetUpdaterClear')) || isTruthy(url.searchParams.get('ricochetCountersUpdaterClear'));

    if (forceRequested || clearRequested) sessionStorage.removeItem(RELOAD_KEY);

    url.searchParams.delete('ricochetCountersUpdaterDebug');
    url.searchParams.delete('ricochetCountersUpdaterForce');
    url.searchParams.delete('ricochetCountersUpdaterClear');
